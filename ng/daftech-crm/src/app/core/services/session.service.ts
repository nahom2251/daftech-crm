...
export class SessionService {
  ...
  private async touch(accountType: SessionAccountType, accountId: string): Promise<void> {
    try {
      await firstValueFrom(this.http.post(`${API_BASE_URL}/sessions/touch`, { accountType, accountId }));
    } catch {
  ...
  async closeSession(accountType: SessionAccountType, accountId: string): Promise<void> {
    this.stopHeartbeat();
    try {
      await firstValueFrom(this.http.post(`${API_BASE_URL}/sessions/close`, { accountType, accountId }));
    } catch {
